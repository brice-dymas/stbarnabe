...
export class CarteDeleteDialogComponent {
    ...
    constructor(private carteService: CarteService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}
...
export class CarteDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ carte }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(CarteDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
                this.ngbModalRef.componentInstance.carte = carte;
