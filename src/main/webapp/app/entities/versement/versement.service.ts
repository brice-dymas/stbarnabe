...
export class VersementService {
    ...
    protected convertDateFromClient(versement: IVersement): IVersement {
        const copy: IVersement = Object.assign({}, versement, {
            dateVersement:
                versement.dateVersement != null && versement.dateVersement.isValid() ? versement.dateVersement.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
        if (res.body) {
            res.body.dateVersement = res.body.dateVersement != null ? moment(res.body.dateVersement) : null;
        }
        return res;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((versement: IVersement) => {
                versement.dateVersement = versement.dateVersement != null ? moment(versement.dateVersement) : null;
            });
        }
        return res;
    }
